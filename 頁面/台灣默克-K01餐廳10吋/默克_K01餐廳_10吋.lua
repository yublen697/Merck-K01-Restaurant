...
--====================連線====================--
	if join > 0 and join < 100000 then					--發送按鈕同步指令
		...
		{command = sendipad.."'connected'"..sendandroid.."'connected'",time="250"},						--發送連線判定
		{command = sendipad.."'connected'"..sendandroid.."'connected'",time="750"},						--發送連線判定
		{command = sendipad.."'connected'"..sendandroid.."'connected'",time="1250"},					--發送連線判定
...
--====================首頁====================--
	if join == 1 and value == 0 then
		if sys == "on" then			--如果系統已開啟，直接進入視訊頁
			buttonstr2 = sendandroid.."'wifi'"..",".."p1200=1"
		else 
			buttonstr2 = sendandroid.."'wifi'"..",".."p1150=1"
		end
	end
--系統開啟
	if join == 10 and value == 0 then
		...
		{command = sendandroid.."'sys_on'",time = "60000"},                                            		 --確認系統已開啟
		}
		for i = 60, 1, -1 do  																	
			seconds = i   																
...
--系統關閉
	if join == 20 and value == 0 then
		...
		delay20 = {
		{command = sendandroid.."'sys_off'",time="1"},														  --確認系統已關閉	
...
--投影機訊號源切換
	...
	if join == 106 and value == 1 then --攝影機
		...
		{command = send24CS..Audioheader.."0006"..Audiobody.."5002"..Audioend,time="500"},
		}
	end
...
--會議主機攝影機
	...
	if join == 5000 and value == 0 then --預設點設置 pq預設點編號
		...
		delay5000 = {
		{command = send24CS..send232ip.."192.168.100.4"..send232data..Polycamera_set..pq_B.."0A"..send232end,time="1"},
	...
	function level_event(join, value)
		...
		if join == 30000 then --麥克風1
			...
			if value >=0 and value <= 100 then	
				clicknum = join + value
				levelstr1 = sendandroid.."'".."levelclick"..clicknum.."'"
			end 
